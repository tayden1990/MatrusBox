...
import { CreateUserDto, LoginDto, TelegramLoginDto, UserProfileDto, TokenRefreshResponseDto } from './dto/auth.dto';
...
export class AuthController {
  ...
  @ApiOperation({ 
    ...
    description: 'Creates a new user account and returns user details with authentication tokens'
  })
  @ApiBody({ 
    type: CreateUserDto,
    description: 'User registration details including email, password, and optional profile information'
  })
  ...
  @ApiOperation({ 
    ...
    description: 'Authenticates users through Telegram integration and creates account if needed'
  })
  @ApiBody({ 
    type: TelegramLoginDto,
    description: 'Telegram authentication data including user ID, username, and validation hash'
  })
  @ApiResponse({ status: 201, description: 'Telegram authentication successful', type: AuthResponseDto })
