...
export class AIController {
  ...
  @ApiOperation({ 
    ...
    description: 'Analyzes audio pronunciation and provides scoring and feedback (currently placeholder implementation)'
  })
  @ApiBody({ 
    type: PronunciationDto,
    description: 'Audio data and text for pronunciation analysis including audio file and target word/phrase'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async scorePronunciation(@Body() dto: PronunciationDto) {
  ...
  @ApiOperation({ 
    ...
    description: 'Uses AI to generate contextual example sentences for vocabulary learning'
  })
  @ApiBody({ 
    type: GenerateExampleDto,
    description: 'Context and parameters for example generation including target word, language, and difficulty level'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ 
    ...
    description: 'Provides AI-generated explanations for words based on their contextual usage'
  })
  @ApiBody({ 
    type: GenerateExplanationDto,
    description: 'Word and context information for generating detailed explanations including usage and meaning'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async generateExplanation(@Body() dto: GenerateExplanationDto) {
    const result = await this.ai.generateExplanation(dto.word, dto.context, dto.language);
    return formatResponse(result);
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Uses AI to generate contextually appropriate synonyms for vocabulary expansion'
  })
  @ApiBody({ 
    type: GenerateSynonymsDto,
    description: 'Target word and language for synonym generation including context for appropriate alternatives'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ 
    ...
    description: 'Provides AI-powered recommendations for optimizing spaced repetition scheduling based on user performance'
  })
  @ApiBody({ 
    type: AdjustSchedulingDto,
    description: 'User performance data including accuracy, streak days, and study patterns for scheduling optimization'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async adjustScheduling(@Body() dto: AdjustSchedulingDto) {
    const result = await this.ai.adjustScheduling(dto.performance, dto.streakDays, dto.averageAccuracy);
    return formatResponse(result);
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Generates AI-powered personalized study recommendations based on user learning patterns and preferences'
  })
  @ApiBody({ 
    type: RecommendationDto,
    description: 'User preferences and context for personalized study recommendations including learning goals and current progress'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async getRecommendation(@Body() dto: RecommendationDto) {
    const result = await this.ai.getRecommendation(dto.userId, dto.context, dto.limit);
    return formatResponse(result);
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Provides AI-generated explanations for user errors to help improve learning and understanding'
  })
  @ApiBody({ 
    type: ErrorExplanationDto,
    description: 'Error details and context for generating helpful explanations including error type and learning context'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ 
    ...
    description: 'Creates a demo flashcard for testing purposes without authentication or AI API calls'
  })
  @ApiBody({ 
    type: GenerateCardDto,
    description: 'Card generation parameters including topic, difficulty level, and language'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  private getDemoCard(topic: string, difficulty: number) {
    const demoCards = {
      ...
      'general': {
        ...
        back: 'Tokyo',
        explanation: 'Tokyo has been the capital of Japan since 1868 and is the largest metropolitan area in the world',
        exampleSentences: ['Tokyo is known for its advanced technology.', 'The Tokyo Olympics were held in 2021.'],
        tags: ['geography', 'capitals', 'japan']
      }
    };

    const cardKey = Object.keys(demoCards).find(key => 
  ...
  @ApiOperation({ 
    ...
    description: 'Creates a complete flashcard with front/back content, examples, and tags using AI based on topic and difficulty'
  })
  @ApiBody({ 
    type: GenerateCardDto,
    description: 'Card generation parameters including topic, difficulty level, and language'
