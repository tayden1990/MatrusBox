...
export class ApiService {
  ...
  async getUserByTelegramId(telegramId: string): Promise<User | null> {
    try {
      const response = await this.api.get<ApiResponse<User | null>>(`/users/telegram/${telegramId}`);
  ...
  async createUserFromTelegram(userData: CreateUserFromTelegramData): Promise<User> {
    const response = await this.api.post<ApiResponse<User>>('/auth/telegram-register', userData);
    return response.data.data as User;
  }

  async getTodaysCards(userId: string): Promise<Card[]> {
    const response = await this.api.get<ApiResponse<Card[]>>(`/study/today?userId=${userId}`);
    return (response.data.data as Card[]) || [];
  }

  async getUserStats(userId: string): Promise<StudyStats> {
    const response = await this.api.get<ApiResponse<StudyStats>>(`/users/${userId}/stats`);
  ...
  async startStudySession(userId: string): Promise<{ sessionId: string }> {
    const response = await this.api.post<ApiResponse<{ sessionId: string }>>('/study/session/start', { userId });
